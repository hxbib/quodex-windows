...
`npm run pack:windows` is the release command. Do not add Electron to root production dependencies.

## Do not

- Check in `node_modules`, `electron/renderer`, `artifacts/`, or unsigned binaries.
- Widen `openExternal` or the OpenAI path allowlist.
- Persist tokens from the renderer.
- Copy Grok App Builder / Better Auth scaffolding into the tracker path.
