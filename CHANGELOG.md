...
- Tauri 2 + system WebView2. Typical download is a ~2 MB zip and a ~5 MB `Quodex.exe` instead of a ~100 MB Electron runtime.
- Vault, tray, flyout, autostart, and the OpenAI allowlist run in Rust. Tokens never enter the usage window.
- Existing `%APPDATA%\Quodex\oauth-tokens.dpapi` files from the Electron build are imported.
- Reset toasts are scheduled in the native process so they still fire while the window is hidden.
- Close / minimize / quit, pin flyout, open-at-login, and the taskbar jump list (Refresh usage) are unchanged.

## 1.0.2

- Portable Electron 32 build on Windows 10 1809+ / Windows 11 x64.
